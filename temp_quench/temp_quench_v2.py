...
def parse_atom_index_selection(selection: str, max_index: int, index_base: int = 1) -> list[int]:
    """
    Parse comma-separated atom indices/ranges into sorted unique 0-based indices.
...
def add_position_restraints(
    ...
    Returns the number of restrained atoms.
    """
    ...
    restraint.setName("HeatingPositionalRestraint")
...
if restrained_template_indices:
    print(
        f"Heating positional restraints requested for {len(restrained_template_indices)} atoms "
        "(per copy) via restraint_idx."
    )
else:
    print("No heating positional restraints requested (restraint_idx is empty).")
...
if restart_from_chk:
    ...
    integrator.setConstraintTolerance(0.00001)
    heating_system = clone_system(full_system)
    if restrained_template_indices:
        ...
        n_restrained = add_position_restraints(
            heating_system, full_positions, restrained_full_indices, restraint_k
        )
        print(
            f"Applied heating positional restraints to {n_restrained} atoms total "
            f"(k={restraint_k})."
        )
    simulation = Simulation(full_topology, heating_system, integrator, platform, properties)
    ...
    if restrained_template_indices:
        print("Removing positional restraints for quenching/production.")
    integrator = LangevinIntegrator(temp_prod, fbsolu, timestep)
    integrator.setConstraintTolerance(0.00001)
    simulation = Simulation(full_topology, full_system, integrator, platform, properties)
